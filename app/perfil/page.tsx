...
export default function PerfilPage() {
  ...
  useEffect(() => {
    const getProfile = async () => {
      // 1. Pega a sessão local (muito mais rápido que getUser)
      const { data: { session } } = await supabase.auth.getSession();
      const user = session?.user;
      
      if (user) {
        setUser(user);
        // Já define o nome vindo do metadata imediatamente
        setName(user.user_metadata?.display_name || '');

        // 2. Busca o perfil em segundo plano para checar a trava
  ...
  const dropAnchor = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const newAnchor: AnchorPoint = {
      ...
      size: Math.random() * 10 + 20, // Tamanhos variados para profundidade
    };
    setAnchors(prev => [...prev.slice(-15), newAnchor]); // Mantém as últimas 15 âncoras
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Atualiza no Auth (Metadata)
      const { error: authError } = await supabase.auth.updateUser({ 
        data: { display_name: name } 
      });
      if (authError) throw authError;

      // 2. Atualiza no Banco de Dados (Profiles)
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-6 overflow-hidden bg-slate-50">
      ...
      >
        ...
        <div className="lg:col-span-5 space-y-8 border-r border-slate-200/50 pr-8">
          <Link href="/">
            <motion.button 
              whileHover={{ x: -5 }}
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              {!hasChangedName && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full bg-slate-900 text-white font-black py-5 rounded-full flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/10"
                  >
                    Salvar Alteração
                    <Save size={18} />
                  </motion.button>
        ...
        {/* COLUNA 2: O PORTO DE ESTABILIDADE */}
        <div className="lg:col-span-7 flex flex-col space-y-6">
          <div 
            ...
            onClick={dropAnchor}
          >
            {/* Cabos de Conexão (Rede de Segurança) */}
              ...
              {anchors.map((anchor) => (
                ...
                >
                  ...
                  <motion.div 
                    ...
                    className="absolute inset-0 border border-emerald-400 rounded-full"
                  />
                </motion.div>
              ))}
            </AnimatePresence>
