...
export default function NavigationDock() {
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('toggleDock', handleToggle);
  }, []);
          ...
          {isHubOpen && (
            <>
              ...
              <motion.div 
                ...
                exit={{ opacity: 0, y: 30 }}
                className="mb-6 w-full bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-2xl pointer-events-auto relative z-10"
              >
        ...
        <nav className="w-full bg-white rounded-[2rem] p-2 shadow-[0_10px_40px_rgba(0,0,0,0.1)] flex items-center justify-around pointer-events-auto h-[4.5rem] border border-slate-100">
          <div className="flex flex-1 justify-around items-center h-full">
            {sideItemsLeft.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`p-3 rounded-full ${isActive ? 'text-emerald-500 bg-emerald-50' : 'text-slate-400'}`}>
          ...
          <div className="relative -mt-8 flex justify-center w-20">
            ...
            >
              {isHubOpen ? <X size={26} strokeWidth={3} /> : <CompassStarIcon className="w-10 h-10" />}
            </button>
          </div>
          ...
          <div className="flex flex-1 justify-around items-center h-full">
            {sideItemsRight.map((item) => {
              const isActive = pathname === item.href;
              return (
                <Link key={item.href} href={item.href} className={`p-3 rounded-full relative ${isActive ? 'text-emerald-500 bg-emerald-50' : 'text-slate-400'}`}>
                  <item.icon size={24} strokeWidth={isActive ? 2.5 : 2} />
